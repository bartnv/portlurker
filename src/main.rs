...
fn setup() -> Option<App> {
    ...
    let mut file = match File::open("config.yml") {
        Ok(file) => file,
        Err(e) => { eprintln!("Unable to open configuration file: {}", e.to_string()); return None; },
    };
    ...
    if !config["general"]["sql_logging"].is_badvalue() {
        ...
        if !config["general"]["nfqueue"].is_badvalue() {
            match config["general"]["nfqueue"].as_i64() {
                ...
                None => { eprintln!("Configuration item 'nfqueue' is not a valid integer"); return None; }
            };
        }
    ...
    fn main() -> Result<(), Box<dyn Error>> {
        let app = Arc::new(RwLock::new(match setup() { // Print initial UI stuff, then parse the config file, and store the config
            Some(app) => app,
            None => return Ok(())
        }));
