...
## Installation
To start, first install the latest stable version of Cargo & rustc by running:

`apt-get install rustc | sh`

`curl -sSf https://static.rust-lang.org/rustup.sh | sh`

Then run `cargo-update | sh` to update the cargo repositaries.

Finally, `cargo build` will build the project and `cargo run` will run it - you may need to do this as root to enable access to lower ports.
